...
export async function getProjectList(context?: string): Promise<string> {
  ...
  try {
    ...
    const namespacesResult = await manager.executeCommand(['get', 'namespaces', '-o', 'json'], { context });
    
    if (!namespacesResult.success) {
      throw new Error(`Failed to get namespaces: ${namespacesResult.error}`);
    }
    
    const namespacesData = typeof namespacesResult.data === 'string' ? JSON.parse(namespacesResult.data) : namespacesResult.data;
    const namespaces = namespacesData.items || [];
    
    // Categorize namespaces
    const systemNamespaces = namespaces.filter((ns: any) => 
      ns.metadata.name.startsWith('openshift-') || 
      ns.metadata.name.startsWith('kube-') ||
      ['default', 'olm'].includes(ns.metadata.name)
    );
    
    const userNamespaces = namespaces.filter((ns: any) => 
      !ns.metadata.name.startsWith('openshift-') && 
      !ns.metadata.name.startsWith('kube-') &&
      !['default', 'olm'].includes(ns.metadata.name)
    );
...
async function getProjectDetails(manager: OpenShiftManager, projectName: string, context?: string) {
  ...
  try {
    // Get resource quotas
    const quotaResult = await manager.executeCommand(['get', 'resourcequota', '-n', projectName, '-o', 'json'], { context });
    if (quotaResult.success) {
      const quotaData = typeof quotaResult.data === 'string' ? JSON.parse(quotaResult.data) : quotaResult.data;
      details.resourceQuotas = quotaData.items?.map((quota: any) => ({
    ...
    // Get limit ranges
    const limitResult = await manager.executeCommand(['get', 'limitrange', '-n', projectName, '-o', 'json'], { context });
    if (limitResult.success) {
      const limitData = typeof limitResult.data === 'string' ? JSON.parse(limitResult.data) : limitResult.data;
      details.limitRanges = limitData.items?.map((limit: any) => ({
    ...
    const podsResult = await manager.executeCommand(['get', 'pods', '-n', projectName, '-o', 'json'], { context });
    if (podsResult.success) {
      const podsData = typeof podsResult.data === 'string' ? JSON.parse(podsResult.data) : podsResult.data;
    ...
    // Get services count
    const servicesResult = await manager.executeCommand(['get', 'services', '-n', projectName, '--no-headers'], { context });
    if (servicesResult.success && servicesResult.data && typeof servicesResult.data === 'string') {
      const serviceLines = servicesResult.data.trim().split('\n').filter((line: string) => line.trim());
    ...
    const deploymentsResult = await manager.executeCommand(['get', 'deployments', '-n', projectName, '--no-headers'], { context });
    if (deploymentsResult.success && deploymentsResult.data && typeof deploymentsResult.data === 'string') {
      const deploymentLines = deploymentsResult.data.trim().split('\n').filter((line: string) => line.trim());
    ...
    // Get routes count
    const routesResult = await manager.executeCommand(['get', 'routes', '-n', projectName, '--no-headers'], { context });
    if (routesResult.success && routesResult.data && typeof routesResult.data === 'string') {
      const routeLines = routesResult.data.trim().split('\n').filter((line: string) => line.trim());
    ...
    const configsResult = await manager.executeCommand(['get', 'configmaps,secrets', '-n', projectName, '--no-headers'], { context });
    if (configsResult.success && configsResult.data && typeof configsResult.data === 'string') {
      const configLines = configsResult.data.trim().split('\n').filter((line: string) => line.trim());
      details.configs = {
...
async function getClusterResourceSummary(manager: OpenShiftManager, context?: string) {
  try {
    ...
    const podsResult = await manager.executeCommand(['get', 'pods', '--all-namespaces', '--no-headers'], { context });
