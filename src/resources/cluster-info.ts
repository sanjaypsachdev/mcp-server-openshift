...
export const clusterInfoResource: Resource = {
  ...
  description: 'Comprehensive information about the OpenShift cluster including version, nodes, and status',
...
export async function getClusterInfo(context?: string): Promise<string> {
  ...
  try {
    ...
    if (nodesResult.success) {
      const nodesData = typeof nodesResult.data === 'string' ? JSON.parse(nodesResult.data) : nodesResult.data;
      ...
      clusterInfo.nodes = {
        ...
        details: nodes.map((node: any) => ({
          ...
          addresses: node.status?.addresses?.map((addr: any) => ({
            type: addr.type,
    ...
    // Get storage classes
    const storageClassResult = await manager.executeCommand(['get', 'storageclass', '-o', 'json'], { context });
    if (storageClassResult.success) {
      const storageData = typeof storageClassResult.data === 'string' ? JSON.parse(storageClassResult.data) : storageClassResult.data;
    ...
    if (eventsResult.success) {
      const eventsData = typeof eventsResult.data === 'string' ? JSON.parse(eventsResult.data) : eventsResult.data;
