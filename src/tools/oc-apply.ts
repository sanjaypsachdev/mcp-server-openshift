...
export const ocApplyTool: Tool = {
  name: 'oc_apply',
  description: 'Apply YAML manifests to OpenShift cluster with comprehensive error handling and validation for all scenarios',
  inputSchema: {
    type: 'object',
    properties: {
      manifest: {
...
export async function handleOcApply(params: OcApplyParams) {
  ...
  try {
    ...
    if (!validationResult.valid) {
      ...
      return formatErrorResponse(progressLog, 'Parameter validation failed', validationResult.error, undefined, validated);
    }
    ...
    if (validated.validate) {
      ...
      if (!preValidationResult.valid) {
        ...
        return formatErrorResponse(progressLog, 'Pre-apply validation failed', preValidationResult.error, undefined, validated);
      }
    ...
    if (validated.dryRun) {
      ...
      if (!dryRunResult.success) {
        ...
        return formatErrorResponse(progressLog, 'Dry run validation failed', dryRunResult.error, undefined, validated);
      }
    ...
    if (!applyResult.success) {
      ...
      return formatErrorResponse(progressLog, 'Apply operation failed', applyResult.error, errorAnalysis, validated);
    }
    ...
    const postValidationResult = await performPostApplyValidation(manager, appliedResources, validated);
...
async function performPreApplyValidation(manager: OpenShiftManager, params: OcApplyParams): Promise<{ valid: boolean; error?: string; warnings: string[] }> {
  const warnings: string[] = [];
  
  try {
    // Check namespace exists
    if (params.namespace && params.namespace !== 'default') {
      const namespaceCheck = await manager.executeCommand(['get', 'namespace', params.namespace], { context: params.context });
      if (!namespaceCheck.success) {
        warnings.push(`Namespace '${params.namespace}' does not exist - it will be created if resources require it`);
      }
    }
    ...
    if (!params.force && params.manifest) {
      ...
      if (conflictCheck.hasConflicts && !params.force) {
        return {
          valid: false,
          error: 'Resource conflicts detected. Use force=true to override, or resolve conflicts manually.',
          warnings
        };
      }
    }
    ...
    if (params.manifest) {
      try {
        ...
        if (!params.manifest.trim().startsWith('apiVersion:') && !params.manifest.trim().startsWith('---')) {
...
async function checkResourceConflicts(manager: OpenShiftManager, params: OcApplyParams): Promise<{ hasConflicts: boolean; warnings: string[] }> {
  const warnings: string[] = [];
  ...
  try {
    ...
    if (params.manifest && params.manifest.includes('name:')) {
      ...
      if (nameMatches) {
        for (const match of nameMatches.slice(0, 3)) { // Check first 3 resources to avoid too many calls
          const name = match.split(':')[1].trim();
          const checkResult = await manager.executeCommand(['get', 'all', '-l', `app=${name}`, '-n', params.namespace], { context: params.context });
    ...
    warnings.push(`Conflict check error: ${error instanceof Error ? error.message : String(error)}`);
  }
...
async function executeDryRun(manager: OpenShiftManager, params: OcApplyParams, command: string[]): Promise<{ success: boolean; data?: string; error?: string }> {
  try {
...
function parseApplyResults(data: string): Array<{ kind: string; name: string; action: string; namespace?: string }> {
...
async function waitForResources(manager: OpenShiftManager, resources: Array<{ kind: string; name: string; action: string }>, params: OcApplyParams): Promise<Array<{ message: string; level: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' }>> {
    ...
    if (resource.action === 'Created' || resource.action === 'Configured') {
      try {
        ...
        if (['Deployment', 'StatefulSet', 'DaemonSet'].includes(resource.kind)) {
          const waitCommand = ['wait', `${resource.kind.toLowerCase()}/${resource.name}`, '--for=condition=Available'];
          if (params.namespace) {
          ...
          if (waitResult.success) {
            results.push({ message: `✅ ${resource.kind}/${resource.name} is ready`, level: 'SUCCESS' });
          } else {
            results.push({ message: `⚠️  ${resource.kind}/${resource.name} not ready: ${waitResult.error}`, level: 'WARNING' });
          }
        } else {
          results.push({ message: `📦 ${resource.kind}/${resource.name} applied (no readiness check available)`, level: 'INFO' });
        }
      } catch (error) {
        results.push({ message: `⚠️  Error waiting for ${resource.kind}/${resource.name}: ${error}`, level: 'WARNING' });
      }
    }
  }
...
async function performPostApplyValidation(manager: OpenShiftManager, resources: Array<{ kind: string; name: string; action: string }>, params: OcApplyParams): Promise<Array<{ message: string; level: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' }>> {
  const results: Array<{ message: string; level: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' }> = [];
  ...
  try {
    ...
    for (const resource of resources.slice(0, 5)) { // Limit to first 5 to avoid too many API calls
      ...
      if (checkResult.success) {
        results.push({ message: `✅ Verified ${resource.kind}/${resource.name} exists`, level: 'SUCCESS' });
      } else {
        results.push({ message: `⚠️  Could not verify ${resource.kind}/${resource.name}: ${checkResult.error}`, level: 'WARNING' });
      }
    }
...
function analyzeApplyError(errorMessage: string, params: OcApplyParams): { category: string; suggestions: string[] } {
...
function formatSuccessResponse(progressLog: string[], params: OcApplyParams, resources: Array<{ kind: string; name: string; action: string }>, result: any) {
  const response = [
...
function formatErrorResponse(progressLog: string[], errorTitle: string, errorDetails?: string, errorAnalysis?: { category: string; suggestions: string[] }, params?: OcApplyParams) {
  const response = [
