...
export const ocDescribeTool: Tool = {
  name: 'oc_describe',
  description: 'Describe any OpenShift resource and share the output in various formats including human-readable summary',
  inputSchema: {
    type: 'object',
    properties: {
      resourceType: {
      ...
      output: {
        ...
        description: 'Output format: text (default oc describe), yaml, json, or human-readable (concise summary)'
      }
    },
...
function generateHumanReadableSummary(resource: any, resourceType: string): string {
  ...
  if (metadata.labels && Object.keys(metadata.labels).length > 0) {
    summary.push(`• Labels: ${Object.entries(metadata.labels).map(([k, v]) => `${k}=${v}`).join(', ')}`);
  }
  
  // Annotations (show only important ones)
  if (metadata.annotations) {
    const importantAnnotations = Object.entries(metadata.annotations)
      .filter(([key]) => !key.startsWith('kubectl.kubernetes.io') && !key.startsWith('deployment.kubernetes.io'))
      .slice(0, 3);
    if (importantAnnotations.length > 0) {
      summary.push(`• Annotations: ${importantAnnotations.map(([k, v]) => `${k}=${v}`).join(', ')}`);
    }
  }
...
function generatePodSummary(spec: any, status: any): string[] {
  ...
  if (spec.containers && spec.containers.length > 0) {
    ...
    spec.containers.forEach((container: any, index: number) => {
      ...
      if (container.ports && container.ports.length > 0) {
        summary.push(`    Ports: ${container.ports.map((p: any) => `${p.containerPort}/${p.protocol || 'TCP'}`).join(', ')}`);
      }
    });
  }
  ...
  if (status.containerStatuses && status.containerStatuses.length > 0) {
    ...
    status.containerStatuses.forEach((cs: any) => {
      ...
      summary.push(`  - ${cs.name}: ${state} (Ready: ${cs.ready ? 'Yes' : 'No'}, Restarts: ${cs.restartCount || 0})`);
    });
  }
...
function generateDeploymentSummary(spec: any, status: any): string[] {
  ...
  summary.push(`• Replicas: ${ready}/${desired} ready, ${available}/${desired} available, ${updated}/${desired} updated`);
  summary.push(`• Strategy: ${spec.strategy?.type || 'Unknown'}`);
  
  // Selector
  if (spec.selector?.matchLabels) {
    summary.push(`• Selector: ${Object.entries(spec.selector.matchLabels).map(([k, v]) => `${k}=${v}`).join(', ')}`);
  }
...
function generateServiceSummary(spec: any, status: any): string[] {
  ...
  if (spec.ports && spec.ports.length > 0) {
    ...
    spec.ports.forEach((port: any) => {
      summary.push(`  - ${port.name || 'unnamed'}: ${port.port}:${port.targetPort}/${port.protocol || 'TCP'}`);
    });
  }
  
  // Selector
  if (spec.selector) {
    summary.push(`• Selector: ${Object.entries(spec.selector).map(([k, v]) => `${k}=${v}`).join(', ')}`);
  }
...
function generateRouteSummary(spec: any, status: any): string[] {
  ...
  if (status.ingress && status.ingress.length > 0) {
    ...
    summary.push(`• Status: ${ingress.conditions?.[0]?.status === 'True' ? 'Admitted' : 'Pending'}`);
...
function generateGenericSummary(spec: any, status: any): string[] {
  ...
  if (spec && Object.keys(spec).length > 0) {
    ...
    Object.entries(spec).slice(0, 5).forEach(([key, value]: [string, any]) => {
  ...
  if (status && Object.keys(status).length > 0) {
    summary.push(`• Status:`);
    Object.entries(status).slice(0, 5).forEach(([key, value]: [string, any]) => {
      if (typeof value === 'object') {
        summary.push(`  - ${key}: [Object with ${Object.keys(value).length} properties]`);
      } else {
        summary.push(`  - ${key}: ${value}`);
