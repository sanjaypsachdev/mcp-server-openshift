...
export const ocDeleteTool: Tool = {
  name: 'oc_delete',
  description: 'Delete OpenShift resources with comprehensive safety checks, validation, and error handling',
  inputSchema: {
    type: 'object',
    properties: {
      resourceType: {
...
export async function handleOcDelete(params: OcDeleteParams) {
  ...
  try {
    ...
    resourcesToDelete.forEach(resource => 
      addProgress(`  - ${resource.kind}/${resource.name} (${resource.namespace || 'cluster-scoped'})`)
    );
    ...
    if (requiresConfirmation(validated, resourcesToDelete) && !validated.confirm && !validated.force) {
    ...
    if (!deleteResult.success) {
      ...
      return formatErrorResponse(progressLog, 'Delete operation failed', deleteResult.error, errorAnalysis);
    }
    ...
    if (validated.wait) {
      ...
      waitResult.forEach(result => 
        addProgress(result.message, result.level)
      );
    }
    
    // Post-delete validation
    addProgress('🔍 Performing post-delete validation...');
    const postValidationResult = await performPostDeleteValidation(manager, deletedResources, validated);
    postValidationResult.forEach(result => 
      addProgress(result.message, result.level)
    );
...
export function validateDeleteParameters(params: OcDeleteParams): { valid: boolean; error?: string } {
  ...
  if (params.resourceType && !params.name && !params.labelSelector && !params.fieldSelector && !params.all) {
    return {
      valid: false,
      error: 'When using resourceType, must specify name, labelSelector, fieldSelector, or all=true'
    };
  }
...
async function performSafetyChecks(manager: OpenShiftManager, params: OcDeleteParams): Promise<{ safe: boolean; warning?: string; warnings: string[] }> {
  ...
  try {
    ...
    if (params.all && !params.labelSelector && !params.fieldSelector) {
      safe = false;
      mainWarning = 'Attempting to delete ALL resources of type without selectors - extremely dangerous';
    ...
    if (params.allNamespaces && (params.all || params.labelSelector)) {
      safe = false;
      mainWarning = 'Attempting to delete resources across ALL namespaces - potentially destructive';
      warnings.push('Consider limiting to specific namespace');
    }
    
    // Check for system namespace operations
    const systemNamespaces = ['kube-system', 'kube-public', 'openshift', 'openshift-monitoring', 'openshift-operators'];
    ...
    const criticalResourceTypes = ['node', 'namespace', 'clusterrole', 'clusterrolebinding', 'customresourcedefinition'];
...
async function discoverResources(manager: OpenShiftManager, params: OcDeleteParams): Promise<Array<{ kind: string; name: string; namespace?: string }>> {
  ...
  try {
    if (params.manifest || params.filename || params.url) {
      ...
      if (getResult.success && getResult.data) {
        const data = typeof getResult.data === 'string' ? JSON.parse(getResult.data) : getResult.data;
...
function requiresConfirmation(params: OcDeleteParams, resources: Array<{ kind: string; name: string; namespace?: string }>): boolean {
  // Require confirmation for dangerous operations
...
async function waitForDeletion(manager: OpenShiftManager, resources: Array<{ kind: string; name: string; action: string }>, params: OcDeleteParams): Promise<Array<{ message: string; level: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' }>> {
  ...
  for (const resource of resources) {
    if (resource.action === 'Deleted') {
      try {
        ...
        const waitCommand = ['wait', `${resource.kind.toLowerCase()}/${resource.name}`, '--for=delete'];
        if (params.namespace) {
        ...
        if (waitResult.success) {
          results.push({ message: `✅ ${resource.kind}/${resource.name} deletion completed`, level: 'SUCCESS' });
        } else {
          results.push({ message: `⚠️  ${resource.kind}/${resource.name} deletion timeout: ${waitResult.error}`, level: 'WARNING' });
        }
      } catch (error) {
        results.push({ message: `⚠️  Error waiting for ${resource.kind}/${resource.name} deletion: ${error}`, level: 'WARNING' });
      }
    }
  }
...
async function performPostDeleteValidation(manager: OpenShiftManager, resources: Array<{ kind: string; name: string; action: string }>, params: OcDeleteParams): Promise<Array<{ message: string; level: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' }>> {
  const results: Array<{ message: string; level: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' }> = [];
  
  try {
    // Verify resources are actually deleted
    for (const resource of resources.slice(0, 5)) { // Limit to first 5 to avoid too many API calls
      if (resource.action === 'Deleted') {
        ...
        if (!checkResult.success && checkResult.error?.includes('not found')) {
          results.push({ message: `✅ Verified ${resource.kind}/${resource.name} is deleted`, level: 'SUCCESS' });
        } else if (checkResult.success) {
          results.push({ message: `⚠️  ${resource.kind}/${resource.name} still exists (may be terminating)`, level: 'WARNING' });
        }
      }
    }
    
    if (resources.length > 5) {
      results.push({ message: `📊 ${resources.length - 5} additional resources not individually verified`, level: 'INFO' });
    }
...
function analyzeDeleteError(errorMessage: string, params: OcDeleteParams): { category: string; suggestions: string[] } {
  ...
  if (errorMessage.includes('forbidden') || errorMessage.includes('Forbidden')) {
    ...
    suggestions.push(`Verify access: oc auth can-i delete ${params.resourceType || '<resource>'} -n ${params.namespace}`);
    ...
    suggestions.push('Remove finalizers manually if safe: oc patch <resource> --type=merge -p \'{"metadata":{"finalizers":null}}\'');
...
function formatSuccessResponse(progressLog: string[], params: OcDeleteParams, resources: Array<{ kind: string; name: string; action: string }>, result: any) {
  const response = [
...
function formatConfirmationResponse(progressLog: string[], resources: Array<{ kind: string; name: string; namespace?: string }>, params: OcDeleteParams) {
  const response = [
    `# ⚠️  Confirmation Required for Delete Operation`,
    ``,
    `## 📦 Resources to be Deleted (${resources.length})`,
    ...resources.slice(0, 20).map(r => `- ${r.kind}/${r.name} ${r.namespace ? `(${r.namespace})` : '(cluster-scoped)'}`),
...
function formatDryRunResponse(progressLog: string[], resources: Array<{ kind: string; name: string; namespace?: string }>, params: OcDeleteParams) {
  const response = [
    `# 🧪 Dry Run: Delete Operation Preview`,
    ``,
    `## 📦 Resources That Would Be Deleted (${resources.length})`,
    ...resources.map(r => `- ${r.kind}/${r.name} ${r.namespace ? `(${r.namespace})` : '(cluster-scoped)'}`),
    ``,
...
function formatErrorResponse(progressLog: string[], errorTitle: string, errorDetails?: string, errorAnalysis?: { category: string; suggestions: string[] }) {
  const response = [
