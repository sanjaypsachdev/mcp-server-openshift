...
async function installOperatorViaOLM(manager: OpenShiftManager, params: OcInstallOperatorParams) {
  try {
    ...
    if (!nameValidation.valid) {
      return {
        content: [
          { type: 'text' as const, text: `Error: Invalid operatorName: ${nameValidation.error}` },
        ],
        isError: true,
      };
    }
    ...
    if (!nsValidation.valid) {
      return {
        content: [
          { type: 'text' as const, text: `Error: Invalid namespace: ${nsValidation.error}` },
        ],
        isError: true,
      };
    }
    if (params.channel !== undefined) {
      ...
      if (!channelValidation.valid) {
        return {
          content: [
            { type: 'text' as const, text: `Error: Invalid channel: ${channelValidation.error}` },
          ],
          isError: true,
        };
      }
    ...
    if (params.version !== undefined) {
      ...
      if (!versionValidation.valid) {
        return {
          content: [
            { type: 'text' as const, text: `Error: Invalid version: ${versionValidation.error}` },
          ],
          isError: true,
        };
      }
