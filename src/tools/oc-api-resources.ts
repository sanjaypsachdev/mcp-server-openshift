...
function formatCategorizedResources(data: string): string {
  ...
  const resources: Array<{ name: string; shortNames: string; apiVersion: string; namespaced: string; kind: string }> = [];
  ...
  const categories = {
    ...
    'Apps & Deployments': resources.filter(r => r.apiVersion.includes('apps/') || r.apiVersion.includes('extensions/')),
    'Networking': resources.filter(r => r.apiVersion.includes('networking') || r.kind.toLowerCase().includes('network')),
    'Storage': resources.filter(r => r.kind.toLowerCase().includes('volume') || r.kind.toLowerCase().includes('storage')),
    'Security & RBAC': resources.filter(r => r.apiVersion.includes('rbac') || r.kind.toLowerCase().includes('role') || r.kind.toLowerCase().includes('security')),
    'OpenShift Specific': resources.filter(r => r.apiVersion.includes('openshift.io') || r.apiVersion.includes('route.openshift.io')),
    'Operators & CRDs': resources.filter(r => r.apiVersion.includes('operators') || r.apiVersion.includes('apiextensions')),
    'Monitoring': resources.filter(r => r.apiVersion.includes('monitoring') || r.kind.toLowerCase().includes('metric')),
    'Other': resources.filter(r => {
      const apiGroup = r.apiVersion.split('/')[0];
      return !['v1', 'apps', 'extensions', 'networking', 'rbac', 'openshift.io', 'route.openshift.io', 'operators', 'apiextensions', 'monitoring'].some(known => 
        apiGroup.includes(known) || r.kind.toLowerCase().includes('volume') || r.kind.toLowerCase().includes('storage') || 
        r.kind.toLowerCase().includes('network') || r.kind.toLowerCase().includes('role') || 
        r.kind.toLowerCase().includes('security') || r.kind.toLowerCase().includes('metric')
      );
    })
  };
